...
**브라우저 Credential 저장**
- 로그인/회원가입 폼은 브라우저의 credential manager를 지원합니다
- 사용자 아이디와 비밀번호가 자동으로 저장/불러오기됩니다
- `autoComplete` 속성이 적절히 설정되어 있어야 작동합니다

**프로필 정보 표시**
- 사용자 이름이 프로필 페이지에 표시됩니다
- `AuthContext`의 `user.name` 필드를 사용합니다
- 로그인 시 백엔드에서 사용자 정보가 반환됩니다
