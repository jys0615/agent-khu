 ...
 * 게시물 캐시 저장
 */
export async function cachePost(postId: string, post: InstagramPost): Promise<void> {
 ...
 * 캐시된 게시물 읽기
 */
export async function getCachedPost(postId: string): Promise<InstagramPost | null> {
    const filePath = path.join(CACHE_DIR, `${postId}.json`);
 ...
 * 모든 캐시된 게시물 가져오기
 */
export async function getAllCachedPosts(): Promise<InstagramPost[]> {
    await ensureCacheDir();
 ...
 * 캐시 인덱스 업데이트
 */
async function updateCacheIndex(): Promise<void> {
    ...
    await fs.writeFile(
        CACHE_INDEX,
        JSON.stringify({ posts, updated_at: new Date().toISOString() }, null, 2),
        "utf-8"
    );
}
